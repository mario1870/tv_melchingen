      ...
      <>
        ...
        >
          ...
          <div className="flex-row hidden lg:flex">
            {navbarData.map((data) => (
              <span className='w-full flex justify-center items-center flex-col pt-4' key={data.titel}>
                  <motion.span>
                    <Link href={data.link} onMouseEnter={() => {setHovered(data.titel)}} onMouseLeave={() => {setHovered(false)}} className={"w-full h-10 md:px-1 xl:px-2 my-1.5 text-white md:text-base xl:text-2xl font-sans font-semibold text-center"}>
                    {data.titel}
                    </Link>
                  </motion.span>    
          ...
          <div className="block lg:hidden">
            {hamburger ? (
              <X onClick={handleToggleHamburger} className="cursor-pointer h-10 w-10" color="white" />
            ) : (
              <Menu onClick={handleToggleHamburger} className="cursor-pointer h-10 w-10 z-50" color='white'/>
            )}
          </div>
        </motion.nav>

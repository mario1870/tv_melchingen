...
const Navbar: React.FC = () => {
  ...
  const [closedButNotOnLoad, setClosedButNotOnLoad] = useState<boolean>(false);
    ...
    <>
          ...
          {navbarData.map((data) => (
            <span className='w-full flex justify-center items-center flex-col pt-4' key={data.titel}>
              <motion.span>
                <Link 
                  to={data.link} 
                  onMouseEnter={() => {setHovered(data.titel)}} 
                  onMouseLeave={() => {setHovered(false)}} 
                  className="w-full h-10 md:px-1 xl:px-2 my-1.5 text-white md:text-base xl:text-2xl font-sans font-semibold text-center"
                >
                  {data.titel}
                </Link>
              </motion.span>    
              <svg className='' width="100%" height="16" viewBox="0 0 145 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                <motion.path 
                  variants={animatedNavbarUnderline} 
                  initial="hidden" 
                  animate={hovered === data.titel ? "visible": "hidden"}  
      ...
      <Suspense fallback={<div>Laden...</div>}>
        {hamburger && (
          ...
          >
            <Sidebar toggle={hamburger} setToggle={setHamburger} />
          </motion.div>
        )}
      </Suspense>
